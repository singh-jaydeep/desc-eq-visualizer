...
from plotting.plot_building import (
    plot_fluxsurf,
    plot_2dsurf_const_rho,
    plot_2dsurf_const_phi,
)
...
def panel_2d_row1(params):
    ...
    return dbc.Row(
        [col1, dropdown1, dropdown2], justify="center", className="mt-3 mb-2"
    )
...
def figure_fluxsurf():
    fig_display = html.Div(
        children=[dcc.Graph(figure={}, id="figure_fluxsurf", mathjax=True)],
        style=borderstyle(),
    )
...
def update_figure_fluxsurf(eq_index, slider_val, params):
    with h5py.File(
        ...
        y = data.attrs["yrange"]
        xdata1 = data[
            "rho_R" + "fluxsurfaces_2d_" + rf"{slider_val}/{params.fx_num_phi-1}"
        ]
        ydata1 = data[
            "rho_Z" + "fluxsurfaces_2d_" + rf"{slider_val}/{params.fx_num_phi-1}"
        ][:]
        xdata2 = data[
            "vartheta_R" + "fluxsurfaces_2d_" + rf"{slider_val}/{params.fx_num_phi-1}"
        ][:]
        ydata2 = data[
            "vartheta_Z" + "fluxsurfaces_2d_" + rf"{slider_val}/{params.fx_num_phi-1}"
        ][:]
        phi_curr = xdata1.attrs["phi_curr"]
        xdata1 = xdata1[:]  ## Load into memory
        fig = plot_fluxsurf(
            xdata1, ydata1, xdata2, ydata2, phi_curr, params, xrange=x, yrange=y
        )
...
def slider_fluxsurf(params):
    ...
    slider = dcc.Slider(
        min=0, max=max, step=None, marks=marks, value=0, id="slider_fluxsurf"
    )
    col = dbc.Row(
...
def figure_2d():
    fig_display = html.Div(
        children=[dcc.Graph(figure={}, id="figure_2d", mathjax=True)],
        style=borderstyle(),
    )
    column = dbc.Col(fig_display, className="mb-3")
    return column
    ...
    if view == "const_rho":
        with h5py.File(
            ...
            data = f[
                ...
                + quantity
                + "constrho_2d_"
                + rf"{slider_val}/{params.surf2d_num_rho}"
            ]
        ...
        with h5py.File(
            ...
            data = f[
                ...
                + quantity
                + "constphi_2d_"
                + rf"{slider_val}/{params.surf2d_num_phi - 1}"
            ]
            lcfs_data = f[
                "/2d/constphi_2d_LCFS_/constphi_2d_LCFS_"
                + rf"{slider_val}/{params.surf2d_num_phi - 1}"
            ]
            label = data.attrs["_label_"]
