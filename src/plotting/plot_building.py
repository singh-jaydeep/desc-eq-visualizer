...
def plot_1d(data, label, params):
    ...
    df = pd.DataFrame(dict(x=rho_grid, y=data))

    labels = {"x": r"$\rho$", "y": ""}
    title = rf"$\text{{Radial profile of }} {label}$"
...
def plot_fluxsurf(
    xdata1, ydata1, xdata2, ydata2, phi_curr, params, xrange=None, yrange=None
):
    fig = go.Figure()
    ...
    for i in range(0, params.fx_num_rho):
        fig.add_trace(
            go.Scatter(
                ...
                y=ydata1[:, i],
                mode="lines",
                line=dict(color="#759eeb", width=2),
            )
        )
    ...
    for i in range(0, params.fx_num_theta):
        fig.add_trace(
            go.Scatter(
                ...
                y=ydata2[:, i],
                mode="lines",
                line=dict(color="#e65566", width=2, dash="dash"),
            )
        )
        fig.update_traces(showlegend=False)
    ...
    fig.update_layout(
        title={
            "text": rf"$\text{{Flux surfaces at toroidal angle }} \phi = {phi_curr:.3f}\pi$",
            "x": 0.5,
            "y": 0.85,
        },
        xaxis=dict(title=dict(text="R (m)")),
    )
    fig.update_layout(
        annotations=[
            {
...
def plot_2dsurf_const_rho(data, label, rho_curr, params):
    ...
    title = rf"${label} \ \text{{at flux surface }} \rho={rho_curr:.3f}$"

    fig = go.Figure()
    fig.add_heatmap(
        autocolorscale=False, colorscale="plasma", y=thetadata, x=zetadata, z=data
    )
    ...
    fig.update_layout(
        ...
        xaxis=dict(title=dict(text=rf"$\phi$")),
    )
    fig.update_layout(
        annotations=[
            {
...
def plot_2dsurf_const_phi(
    ...
    title = rf"${label} \ \text{{at toroidal angle }} \phi={phi_curr:.3f}\pi$"
...
def plot_3dmagax(eq_index, curve_data, hovertemplate, params):
    ...
    fig = go.Figure(
        data=[
            go.Scatter3d(
                x=curve_data[0], y=curve_data[1], z=curve_data[2], mode="lines"
            )
        ]
    )

    fig.update_traces(customdata=df, hovertemplate=hovertemplate)

    fig.update_layout(title={"text": rf"$\text{{Magnetic axis}}$", "x": 0.5, "y": 0.9})
    fig.update_layout(
        scene=plot_params["scene"],
        autosize=False,
...
def plot_3dsurf(
    ...
    title = rf"${label} \ \text{{at flux surface }} \rho={rho_curr:.3f}$"
    if quantity == "1":
        title = rf"$\text{{Flux surface at }} \rho={rho_curr:.3f}$"
    ...
    if quantity == "1":
        fig.update_traces(opacity=0.4, showscale=False)
    else:
        fig.add_annotation(
            text=colorbartitle,
            showarrow=False,
