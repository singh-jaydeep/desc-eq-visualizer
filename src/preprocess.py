...
def preprocess():
    ...
    if num_eq == 0:
        ...
        for i in range(0, len(params.eq_loaded)):
            ...
            fluxsurface_mask = compute_2dfluxsurfaces(eq, f, subgp_2d_constphi_LCFS, subgp_2dflux, subgp_2d_constphi, params)
            compute_2dprofiles(eq, f, subgp_2d_constphi, subgp_2d_constrho, fluxsurface_mask, params)
...
def compute_2dfluxsurfaces(eq, f, gp_constphi_LCFS, gp_2dflux, gp_2d_constphi, params):
    ...
    for j in range(0,params.surf2d_num_phi):
        ...
        dset_2dconstphi_LCFS = gp_constphi_LCFS.create_dataset("constphi_2d_LCFS_" + rf"{j}/{params.surf2d_num_phi - 1}", data=data_LCFS, compression='gzip')
        dset_2dconstphi_LCFS.attrs['_phi_curr_'] = round(2/(params.surf2d_num_phi*eq.NFP) * j,3)
    ...
    for j in range(0,params.fx_num_phi):
        ...
        dset_2dflux_A = gp_2dflux.create_dataset("rho_R"+"fluxsurfaces_2d_"+ rf"{j}/{params.fx_num_phi-1}", data=xdata1.astype(np.float32), compression='gzip')
        dset_2dflux_A.attrs['label'] = "Flux surface"
        dset_2dflux_A.attrs['phi_curr'] = np.round(2*j/(params.fx_num_phi*eq.NFP),3)

        dset_2dflux_B = gp_2dflux.create_dataset("rho_Z"+"fluxsurfaces_2d_"+ rf"{j}/{params.fx_num_phi-1}", data=ydata1.astype(np.float32), compression='gzip')
        dset_2dflux_B.attrs['label'] = "Flux surface"
        dset_2dflux_B.attrs['phi_curr'] = np.round(2*j/(params.fx_num_phi*eq.NFP),3)

        dset_2dflux_C = gp_2dflux.create_dataset("vartheta_R"+"fluxsurfaces_2d_"+ rf"{j}/{params.fx_num_phi-1}", data=xdata2.astype(np.float32), compression='gzip')
        dset_2dflux_C.attrs['label'] = "Flux surface"
        dset_2dflux_C.attrs['phi_curr'] = np.round(2*j/(params.fx_num_phi*eq.NFP),3)

        dset_2dflux_D = gp_2dflux.create_dataset("vartheta_Z"+"fluxsurfaces_2d_"+ rf"{j}/{params.fx_num_phi-1}", data=ydata2.astype(np.float32), compression='gzip')
        dset_2dflux_D.attrs['label'] = "Flux surface"
        dset_2dflux_D.attrs['phi_curr'] = np.round(2*j/(params.fx_num_phi*eq.NFP),3)

        if j==0:
            fig = pbuild.plot_fluxsurf(xdata1,ydata1,xdata2,ydata2,0,params, xrange=xrange, yrange=yrange)
...
def compute_2dprofiles(eq, f, gp_2d_constphi, gp_2d_constrho, fluxsurface_mask, params):
    ...
    for q in params.attrs_2d:
        ...
        for j in range(0,params.surf2d_num_rho+1):
            ...
            dset_2dconstrho = gp_2d_constrho.create_dataset(q+"constrho_2d_"+rf"{j}/{params.surf2d_num_rho}", data=data.astype(np.float32), compression='gzip')
            ...
            if q == params.attrs_2d[0] and j == params.surf2d_num_rho:
                fig = pbuild.plot_2dsurf_const_rho(data,params.attrs_dict[q]['label'], 1, params)
        ...
        for j in range(0,params.surf2d_num_phi):
            ...
            dset_constphi_2d_ = gp_2d_constphi.create_dataset(q+"constphi_2d_" + rf"{j}/{params.surf2d_num_phi - 1}", data=data_2d_, compression='gzip')
            ...
            dset_constphi_2d_.attrs['_phi_curr_'] =  round(2/(params.surf2d_num_phi*eq.NFP) * j,3)
        ...
        for j in range(1,params.surf3d_num_rho+1): ## Won't evaluate rho=0, i.e. the axis
            ...
            if q == '1': ## Only store the mesh once
                ...
                if j == params.surf3d_num_rho:
                    fig = pbuild.plot_3dsurf(data_3d_[q], data_mesh_3d_, eq_index, '1', params.surf3d_num_rho, params, 
